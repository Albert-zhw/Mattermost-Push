...
如果您想使用**固定格式**（标题固定为 📬 Mattermost，内容固定为 📧 New Message + 时间），请参考以下说明：

**使用固定格式**：

1. 编辑 `mattermost-listener.py`
2. 找到 `send_to_huixiao()` 函数中的 `push_data` 字典（约第 55 行）
3. 修改为：
...
push_data = {
    ...
    'title': CONFIG['hui xiao_title'],  # 固定标题：📬 Mattermost
    'content': f"{CONFIG['hui xiao_content']} {current_time}",  # 固定内容 + 时间
    'icon': CONFIG['hui xiao_icon']
}
```

确保 CONFIG 配置中有以下项（约第 23-24 行）：
...
push_data = {
    'cid': CONFIG['hui xiao_cid'],
    'group': CONFIG['hui xiao_group'],
    'title': f'📬 {sender_name}',  # 显示发件人
    'content': f'{message} ({current_time})',  # 显示完整消息
    'icon': CONFIG['hui xiao_icon']
}
```
...
在 Mattermost 中给自己发送一条私聊消息，您的 HarmonyOS 设备应该立即收到推送！
...
```
标题：📬 Mattermost
内容：📧 New Message 2026-03-22 14:30:45
```
